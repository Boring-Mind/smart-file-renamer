...
SUPPORTED_TAGS = {
    'artist', 'album', 'title', 'track', 'disc', 'year', 'genre',
}

def parse_desired_to_template(desired: str, slot_values_from_old0: List[str], verbose: bool = False) -> Template:
    """
...
def render_with_tags(tpl: Template, slot_values: List[str], tags: Dict[str, str], case_mode: str) -> str:
...
def main(argv=None):
    ...
    ap.add_argument('--dir', required=True, help='Target directory containing the files to rename')
    ap.add_argument('--olds', nargs='+', required=True,
    ...
    ap.add_argument('--old-pattern', help='Provide a custom pattern if you see that auto-generated pattern is not precise.')
    ap.add_argument('--dry-run', action='store_true', help='Show what would happen, do not rename')
    ap.add_argument('--conflict', choices=['skip', 'overwrite', 'unique'], default='unique',
                    help='Collision handling when target exists')
    ap.add_argument('--ext-mode', choices=['keep', 'desired'], default='keep',
                    help="Keep original extensions or use the desired example's extension for all")
    ap.add_argument('--case-slots', choices=['asis', 'lower', 'upper', 'title', 'slug', 'underscore'], default='asis',
                    help='Apply casing/normalization to slot values and tag values')
    ap.add_argument('--mkdirs', dest='mkdirs', action='store_true', default=True,
    ...
    ap.add_argument('--include-hidden', action='store_true', help='Include dotfiles in scans')
    ...
    if slot_vals_old0 is None:
        print("! Could not parse the first example old with the learned OLD template. Give more/cleaner examples.", file=sys.stderr)
        return 3
    ...
    if not candidates:
        print("No files in the folder matched the learned OLD template. Refine your examples.")
        return 0
    ...
    sym = {'noop': ' = ', 'rename': ' → ', 'skip': ' × ', 'overwrite': ' ↻ ', 'unique': ' ⊕ '}
    for src, dst, act in resolved:
        print(f"  {os.path.relpath(src, dirpath)}{sym[act]}{os.path.relpath(dst, dirpath)}")
    ...
    print(f"Done. Renamed: {ran}, Overwritten: {overwritten}, Unique-renamed: {uniqued}, Skipped: {skipped}.")
    return 0
